...
void nRF_init( void )
{
	...
	#if USE_IRQ == 1
	Initialize_INTERRUPT_For_nRF(); //initialize INTx 
	#endif

	nRF_SET_Transmitter_Adres(PSTR("ATNEL"));		
	nRF_SET_Reciver_Addres(RX_ADDR_P0, PSTR("ATNEL"));	
	nRF_Config_Register( CONFIG, nRF24L01_CONFIG);         	
	nRF_Clear_RX();                                       	
	nRF_Clear_TX();                                         
	nRF_Set_State_And_Width_Of_CRC( ONE_BYTE , ON );        
	nRF_Set_Channel(10);                                    
    	nRF_Set_Active_DataPipe_And_ACK ( ERX_P0, ON, ACK_ON )  
	nRF_Set_Retransmission_Time_And_Ammount(WAIT_4000uS , RETR_15_TIMES );  

	nRF_Set_Data_Speed_And_Reciver_Power(TRANS_SPEED_250kB, RF_PWR_0dB );   
	...
	nRF_Config_Register(STATUS, (1<<TX_DS) | (1<<RX_DR) | (1<<MAX_RT));    
